...
fn test_grid_data_new_all_false() {
    let mut d = grid_data_new(4, 3);
    for row in 0..3i32 {
        for col in 0..4i32 {
            assert!(!grid_data_get(&mut d, row, col, 4));
        }
    }
}
...
fn test_grid_data_set_get() {
    ...
    assert!(grid_data_get(&mut d, 2, 3, 5));
    assert!(!grid_data_get(&mut d, 2, 4, 5));
    assert!(!grid_data_get(&mut d, 3, 3, 5));
}
...
fn test_grid_data_set_multiple_cells() {
    ...
    assert!(grid_data_get(&mut d, 0, 0, 6));
    assert!(grid_data_get(&mut d, 1, 2, 6));
    assert!(grid_data_get(&mut d, 3, 5, 6));
    assert!(!grid_data_get(&mut d, 0, 1, 6));
    assert!(!grid_data_get(&mut d, 2, 3, 6));
}
